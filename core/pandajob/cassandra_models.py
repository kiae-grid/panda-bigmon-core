...
CREATE TABLE panda_archive.day_mtime_site_errors_cnt_30m (
    date timestamp,
    base_mtime timestamp,
    computingsite text,
    errcode text,
    ...
    err_count int,
    job_count int,
    PRIMARY KEY (date, base_mtime, computingsite, errcode, diag)
...
class day_mtime_site_errors_cnt_30m(Model):
    date = columns.DateTime(primary_key = True)
    base_mtime = columns.DateTime(primary_key = True)
    computingsite = columns.Text(primary_key = True)
    errcode = columns.Integer(primary_key = True)
...
# ) WITH CLUSTERING ORDER BY (base_mtime ASC, computingsite ASC, errcode ASC, diag
#  ASC)
#     AND bloom_filter_fp_chance = 0.01
#     AND caching = '{"keys":"ALL", "rows_per_partition":"NONE"}'
#     AND comment = ''
#     AND compaction = {'min_threshold': '4', 'class': 'org.apache.cassandra.db.co
# mpaction.LeveledCompactionStrategy', 'max_threshold': '32'}
...
"""    
CREATE TABLE panda_archive.day_errors_cnt (
    date timestamp,
    interval text,
    base_mtime timestamp,
    "count" counter,
    PRIMARY KEY ((date, interval), base_mtime)
) WITH CLUSTERING ORDER BY (base_mtime ASC)
    AND bloom_filter_fp_chance = 0.01
    AND caching = '{"keys":"ALL", "rows_per_partition":"NONE"}'
    AND comment = ''
